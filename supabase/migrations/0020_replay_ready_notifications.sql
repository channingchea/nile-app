...
-- Idempotent enough for practical use: a unique (recipient, type, entity)
-- partial guard isn't enforced at the table level, so we rely on this being
-- called once per egress_ended. If a replay row is re-marked ready, callers
-- should not re-invoke. p_event_id is the events PK.
