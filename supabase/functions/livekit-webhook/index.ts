...
serve(async (req) => {
  ...
  if (event.event === "egress_ended" && event.egressInfo) {
    ...
    // file result carries the finalized location + duration (ns → ms).
    ...
    // server-side by each recipient's preference. Non-fatal — a failed fanout
    // must not make LiveKit retry the whole webhook.
